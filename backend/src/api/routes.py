...
@router.get('/user_products', response_model=list[schemas.UserProductOutput], response_model_by_alias=False)
async def list_user_products(user: User = Depends(get_user), session: AsyncSession = Depends(db_session)):
    return await services.list_user_products(session, user.id)


@router.post('/user_products', status_code=201, response_model=schemas.UserProductOutput, response_model_by_alias=False)
...
async def delete_user_product(user_product_id: int, session: AsyncSession = Depends(db_session)) -> None:
